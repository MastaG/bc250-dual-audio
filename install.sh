...
echo "BC-250 Dual Audio v0.11 installer"
...
if [[ -f "$STOCK_ALSA" && "${BC250_ALLOW_UNTESTED_WP:-0}" != "1" ]]; then
  ...
  if [[ "$STOCK_ALSA_SHA256" != "$EXPECTED_STOCK_ALSA_SHA256" ]]; then
    echo "ERROR: distro stock alsa.lua does not match the WirePlumber 0.5.17 base used by v0.11." >&2
...
# Back up both old prototype files and all host-wide files replaced by v0.11.
